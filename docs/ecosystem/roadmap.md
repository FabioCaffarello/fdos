...
| M9 | **Resolution** — who mints, when, on whose authority; per-scheme canonicalisation before `Derive`. Track B (agent knowledge and harness) delivered in-milestone | Track A delivered ([PR #51](https://github.com/FabioCaffarello/fdos/pull/51), merged 2026-08-07); the mint loop's open items stay in [#57](https://github.com/FabioCaffarello/fdos/issues/57) and keep the milestone open |
| M9.5 | Governance ops — PREVC adopted (ADR-0031), blocked register moved to issues (ADR-0032), issue templates, hygiene | ✅ gate [PR #52](https://github.com/FabioCaffarello/fdos/pull/52) merged; milestone closed |
| M10 | **Persistence** — the ledger event store: durable facts, bitemporal as-of reads, reproducible. RFC before code | ✅ gate [PR #58](https://github.com/FabioCaffarello/fdos/pull/58), delivery [#60](https://github.com/FabioCaffarello/fdos/pull/60); `libs/ledger-sqlite/v0.1.0` published ([#66](https://github.com/FabioCaffarello/fdos/issues/66) records the tag correction) |
| M11 | **Transport** — first composition root in `apps/`: a submission service over `fdos.ingest.v1`; makes D2 a live decision | delivered in substance — plan gate [PR #63](https://github.com/FabioCaffarello/fdos/pull/63), `apps/submitd` ([PR #71](https://github.com/FabioCaffarello/fdos/pull/71)), ADR-0038. Of its two preconditions, the "never a service" clause was answered by ADR-0037 superseding ADR-0029; **D2 was not met** — the listener ships a loopback guard, not an answer ([#64](https://github.com/FabioCaffarello/fdos/issues/64)). the release decision stays a draft ([PR #72](https://github.com/FabioCaffarello/fdos/pull/72)) |
| M12 | **Consumer enablement** — validator binary and conformance kit distributed; unblocks `fdos-connectors` C4 | after M11; the D4 precondition is met — closed by ADR-0028 via RFC-0011 ([#28](https://github.com/FabioCaffarello/fdos/issues/28) closed). No plan exists yet; `examples/ingest` (PR #47) pre-delivers part of the kit |
